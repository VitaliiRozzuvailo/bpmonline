    ...
    return {
        ...
        methods: {
            ...
            onUpdateTTNClick: function () {
                ...
                //TODO city REF
                var city = activeOrder.get("SxCity").displayValue;// город TODO - № отделения
                ...
                bq.execute(function (result) {
                    ...
                    for (var i = 0; i < products.length; i++) {
                        ...
                        bq.execute(function (result) {
                            ...
                            this.createRequestToNP(weight, fName, lName, city, price, date, phoneR, th);
            ...
            //получение веса 
            createRequestToNP: function (weight, fName, lName, city, price, date, phoneR, th) {
                ...
                select.addColumn("PaymentType.Id", "PaymentType");
                select.addColumn("SxNumberOfDepartament.Name", "SxNumberOfDepartament");
                select.addColumn("SxIsAddress", "SxIsAddress");
                select.addColumn("SxIsRedelivery", "SxIsRedelivery");
                select.addColumn("SxStreet", "SxStreet");
                select.addColumn("SxBuild", "SxBuild");
                select.addColumn("SxFlat", "SxFlat");
                ...
                bq.execute(function (result) {
                    ...
                    var seatsAmount = order[0].SxCountOfPlacesNP; //количество мест
                    /**/
                    ...
                    var serviceType = getServiceType(order[0].SxDeliveryTypeNP); //тип доставки
                    /**/
                    var paymentMethod = order[0].PaymentType === "c2d88243-685d-4e8b-a533-73f4Cc8e869b" ? "NonCash" : "Cash";

                    //TODO set refAddressRecipient
                    var refAddress = order[0].SxNumberOfDepartament.replace(/\"/gi, '\\"');;

                    var isAddress = order[0].SxIsAddress;
                    var isRedelivery = order[0].SxIsRedelivery;
                    var street = order[0].SxStreet;
                    var build = order[0].SxBuild;
                    var flat = order[0].SxFlat;
                    ...
                    ServiceHelper.callService("SxNovaPochtaApi", "CreateOrderInNP",
                        function (response) {
                            ...
                            if (th.parseXMLfromNP(th.stringToXML(result), th))
